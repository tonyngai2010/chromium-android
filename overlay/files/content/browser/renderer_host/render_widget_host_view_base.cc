...
// 鍦ㄦ墍鏈?#include 涔嬪悗锛宯amespace content 涔嬪墠娣诲姞锛?
// ===== SPOOF-RES BEGIN (anon namespace placed before namespace content) =====
namespace {
struct SpoofParams {
  bool enabled = false;
  int w = 0;
...
// "1080x1920@2.5" -> 1080, 1920, 2.5
static bool ParseWxH_DPR(const std::string& s, int* w, int* h, float* dpr) {
  int iw = 0, ih = 0; double id = 0.0;
  if (std::sscanf(s.c_str(), "%dx%d@%lf", &iw, &ih, &id) == 3 && iw > 0 && ih > 0 && id > 0.0) {
    *w = iw; *h = ih; *dpr = static_cast<float>(id); return true;
  }
  return false;
}
static SpoofParams ParseSpoofParams() {
  ...
  if (cmd && cmd->HasSwitch(switches::kSpoofResolution)) {
    std::string v = cmd->GetSwitchValueASCII(switches::kSpoofResolution);
    if (ParseWxH_DPR(v, &sp.w, &sp.h, &sp.dpr)) { sp.enabled = true; return sp; }
  }
  // 榛樿鍏滃簳锛屽彲鎸夐渶鍏抽棴
  sp.enabled = true; sp.w = 888; sp.h = 888; sp.dpr = 2.0f;
  return sp;
}
}  // namespace
// ===== SPOOF-RES END =====
...
void RenderWidgetHostViewBase::UpdateScreenInfo() {
  ...
  if (host()) {
    ...
    // ScreenInfo info = screen_infos_.current(); // 鑾峰彇褰撳墠灞忓箷淇℃伅
    // static SpoofParams sp = ParseSpoofParams();
    // if (sp.enabled) {
    // info.rect_in_pixels = gfx::Rect(0, 0, sp.w, sp.h);
    // const int inset_px = static_cast<int>(24 * sp.dpr); /
    // info.available_rect = gfx::Rect(0, inset_px, sp.w, sp.h - inset_px);
    // info.device_scale_factor = sp.dpr;
    // }
...
void RenderWidgetHostViewBase::SynchronizeVisualProperties() {
  static SpoofParams sp = ParseSpoofParams();
  if (sp.enabled && sp.w > 0 && sp.h > 0 && sp.dpr > 0.f) {
    const int css_w = static_cast<int>(sp.w / sp.dpr);
    const int css_h = static_cast<int>(sp.h / sp.dpr);
    // 闇€瑕佹椂鍦?Android/Host 灞傛嵁姝よ鍐?ScreenInfo/Viewport锛汢ase 灞備笉鐩存帴鏀圭粨鏋勪綋
    (void)css_w; (void)css_h;
  }
  if (host())
    host()->SynchronizeVisualProperties();
}
